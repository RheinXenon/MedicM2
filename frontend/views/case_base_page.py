...
def render_case_base_page():
    ...
    if hospital:
        ...
        for dept_id, case_base in hospital.department_case_bases.items():
            # 安全获取病例数量
            try:
                case_count = len(case_base.cases) if hasattr(case_base, 'cases') else 0
            except:
                case_count = 0
        ...
        if dept_options:
            ...
            if dept_filter == "全部":
                ...
                for case_base in hospital.department_case_bases.values():
                    try:
                        if hasattr(case_base, 'cases') and isinstance(case_base.cases, list):
                            cases.extend(case_base.cases)
                            if len(cases) >= 20:
                                break
                    except Exception as e:
                        st.warning(f"读取病例时出错: {e}")
                        continue
                cases = cases[:20]
            else:
                # 获取特定科室的病例
                dept_id = dept_id_to_name[dept_filter]
                case_base = hospital.department_case_bases.get(dept_id)
                if case_base and hasattr(case_base, 'cases') and isinstance(case_base.cases, list):
